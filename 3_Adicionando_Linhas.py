...
df_compras = pd.read_csv(f"{caminho_dataset}/compras.csv", sep=";", decimal=",", index_col=0)
...
lista_lojas = df_lojas["cidade/estado"].tolist()
loja_selecionada = st.sidebar.selectbox("Selecione a loja", lista_lojas)
 
lista_vendedores = df_lojas.loc[
    df_lojas["cidade/estado"] == loja_selecionada, "vendedores"].iloc[0]
...
if st.sidebar.button("Adicionar Nova Compra"):
    ...
    df_compras.to_csv(f"{caminho_dataset}/compras.csv", index=False, decimal=",", sep=";")
